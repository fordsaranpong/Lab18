...
int main(){
	...
	while(getline(student_file,textline)){
		...
		char format[]={"%[^,],%d,%c,%lf"};
		student s; 
		const char * textC = textline.c_str();
	    sscanf(textC,format,name2,&s.id,&s.gender,&s.gpa);//Use sscanf() to split the values in textline and assign those values to the members of struct s;
	    s.name = name2;
